    ...
    schema.post('findOne', (result) => {
        ...
        if (lastItem) {
            if (lastItem.year === moment().year() && lastItem.week === moment().isoWeek() && lastItem.day === moment().isoWeekday()) {
                ...
                mongoose.model('Song').findOneAndUpdateAsync({ '_id': ObjectId(result._id), 'meta.stats': { '$elemMatch': { 'year': moment().year(), 'week': moment().isoWeek(), 'day': moment().isoWeekday() } } }, { '$inc': { 'meta.stats.$.views': 1, 'meta.totalViews': 1 } }).then((res, err) => { if (err) logger.error(err); })
            }
            else {
                const stats = {
                    year: moment().year(),
                ...
                mongoose.model('Song').updateOneAsync({ '_id': ObjectId(result._id) }, { '$addToSet': { 'meta.stats': { stats } } })
            }
        } else {
            var meta = newMeta()
